...
const TUTORIAL_STEPS = [
  ...
  },
  {
    ...
    title: '添加到桌面',
    description: '长按链接选择「添加到桌面」，系统会弹出确认对话框，确认后即可在手机桌面生成独立快捷方式图标（需 Android 8.0+）。',
    color: '#FF6B6B',
  },
];
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <TouchableOpacity style={styles.skipButton} onPress={handleSkip}>
        <Text style={[styles.skipText, { color: colors.textTertiary }]}>跳过</Text>
      </TouchableOpacity>

      <View style={styles.content}>
        <View style={[styles.iconOuter, { backgroundColor: colors.surface }]}>
          <View style={[styles.iconInner, { backgroundColor: step.color + '20' }]}>
          ...
          {TUTORIAL_STEPS.map((_, index) => (
            <View
              ...
              style={[
                ...
                index === currentStep && { width: 24, backgroundColor: colors.primary },
              ]}
            />
          ))}
        </View>
...
const styles = StyleSheet.create({
  ...
  skipText: { fontSize: FONT_SIZE.md, fontWeight: '500' },
  content: {
    flex: 1,
    alignItems: 'center',
  ...
  nextText: { fontSize: FONT_SIZE.lg, fontWeight: '700' },
});
